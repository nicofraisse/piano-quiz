...
const FinalScore = (props) => {
  const trueAnswers = props.attemptData
    .flat()
    .filter((data) => data.correct === true)
  const percentAccuracy = Math.round(
    (trueAnswers.length / props.attemptData.flat().length) * 100,
    1
  )
    ...
    return (
      <div key={index} style={{ margin: '0 24px' }}>
        ...
        {round.map((attempt, index) => {
          return (
            <p key={index} style={{ textAlign: 'right', fontSize: 20 }}>
  ...
  return (
    <div className={classes.FinalScore} onClick={props.click}>
      <div>
        <h2>
          Well done! You finished this quiz with <span>{percentAccuracy}%</span>{' '}
          accuracy.
        </h2>
        <div>{scoreDataDetails}</div>
        <p style={{ fontSize: 16, color: 'rgba(160, 160, 160)' }}>
