...
const App = () => {
  ...
  useEffect(() => {
    if (window.location.pathname[1] === 'S') {
      setActiveQuiz(
        JSON.parse(decodeURIComponent(window.location.pathname.slice(2)))
      )
    }
  }, [])
  ...
  return (
    <div className='App'>
      ...
      {!freeplayShowing && !dashboardShowing && !activeQuiz ? (
        <LandingPage click={showDashboard} />
      ) : null}
      ...
      {dashboardShowing ? (
        <Dashboard
          ...
          submitQuiz={(event) => handleQuizSubmit(event)}
        />
      ) : null}

      {activeQuiz ? <Game quiz={activeQuiz} /> : null}
    </div>
  )
}
